...
# --- ΣΥΣΤΗΜΑ ΑΣΦΑΛΕΙΑΣ (LOGIN SCREEN) ---
...
# --- ΔΙΑΧΕΙΡΙΣΗ PAYWALL ΚΑΙ ΕΙΣΑΓΩΓΗΣ ΜΗΝΥΜΑΤΟΣ ---
if st.session_state.prompt_count >= 3:
    ...
    if st.button("💎 UNLOCK UNLIMITED ACCESS ($4.99/mo)"):
        st.balloons()
        st.toast("Redirecting to secure gateway...", icon="💳")
else:
    if user_input := st.chat_input("Input data vector or dilemma for analysis..."):
        ...
        with st.chat_message("assistant", avatar="🧠"):
            ...
            loading_phrases = [
                "Isolating emotional noise...", 
                "Calculating pro/con vectors...", 
                "Structuring objective matrices...", 
                "Compiling brutal conclusion..."
            ]
            ...
            if ai_response:
                ...
                status_placeholder.error("⚠️ Connection timeout. Server overloaded. Please try again.")
                st.session_state.prompt_count -= 1  
                
            st.rerun()

# Στατιστικά Συστήματος στο Κάτω Μέρος (Metrics)
st.write("---")
c1, c2, c3 = st.columns(3)
with c1: st.metric(label="System Mood", value="0% Empathy", delta="Cold Facts")
